	...
	function page_index(){
		...
		$this->add('VirtualPage')
			...
			->set(function($p)use ($status) {
				...
				$account_model->addExpression('PanNo')->set($account_model->refSQL('member_id')->fieldQuery('PanNo'));
				...
				$grid->addSno();
				$grid->setModel($account_model,array('member_no','ActiveStatus','sm_no','created_at','AccountNumber','scheme','Amount','PanNo','adhaar_no','member','father_name','address','phone_no','agent','dealer','mo','team','agent_saving_acc','agent_phone_no','Nominee','NomineeAge','RelationWithNominee','repayment_mode','bank_account_number','bank_name','bank_IFSC','bank_branch'));
